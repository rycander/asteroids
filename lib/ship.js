(function (){
  ...
  Ship.prototype.move = function (){
    if(this.wdown) {
      accel = Util.vecFromAngle(0.4, -this.rotation * Math.PI);
      this.vel = [
        this.vel[0] + accel[0],
        this.vel[1] + accel[1]
      ];
    ...
    if(this.spacedown) {
      accel = Util.vecFromAngle(-0.016 * this.laserPower, -this.rotation * Math.PI);
      this.vel = [
        this.vel[0] + accel[0],
        this.vel[1] + accel[1]
      ];
      ...
      this.laserStart = Util.vecAdd(
        this.pos,
        Util.vecFromAngle(this.radius, -this.rotation * Math.PI)
      );
      var lasVec = Util.vecFromAngle(
      ...
      this.vecNorm = Util.boundVec(this.vecNorm, this.laserPower);
      ...
      --this.chargeDelay;
      if (this.laserPower < this.radius * 5 && this.chargeDelay <= 0) {
        ++this.laserPower;
      }
    }
